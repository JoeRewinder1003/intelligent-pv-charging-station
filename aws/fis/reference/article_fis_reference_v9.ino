/*
  ...
  Final article simulation version: high-demand, three-output utilization, restriction/fault-state logging, and anti-chattering dwell-time control
  -------------------------------------------------------
  Purpose:
...
void applyStableModeToOutputs() {
  // Anti-chattering / dwell-time logic:
  // - Service increases are never applied immediately; they must remain stable.
  // - Normal FIS decreases are also filtered to avoid rapid ON/OFF switching.
  // - Safety reductions bypass the filter and are applied immediately.
  //
  // This makes the implemented mode more realistic for relays, SSRs, DC/DC converters,
  // and user-facing charging outputs.
  bool safetyReduction =
    (faultState >= 2) ||                         // data fault or critical fault
    (outputsBlocked == 1) ||                     // charging explicitly blocked
    ((faultState > 0) && (requestedMode < appliedMode)); // restriction reduces mode

  operatingMode = stabilizeOperatingMode(requestedMode, safetyReduction);
